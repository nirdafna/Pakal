...
3. **Claude does not merge. Nir merges.** Report the review outcome, hand over the PR URL,
   stop. A clean review is not consent, and a framework whose default is "the orchestrator
   merges" does not override this.
...
Rules 1 and 3 are enforced mechanically by `.claude/hooks/enforce-workflow.sh`. The rest
depend on you actually following them.
